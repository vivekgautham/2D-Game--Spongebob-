  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sounds/UW.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load UW.mp3")+Mix_GetError();

  startMusic();
  sounds.push_back( Mix_LoadWAV("sounds/laserfire3.wav") );
  sounds.push_back( Mix_LoadWAV("sounds/liquidblast.wav") );
  sounds.push_back( Mix_LoadWAV("sounds/implosion1.wav") );
